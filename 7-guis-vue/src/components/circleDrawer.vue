<template>
  <div class="flex flex-col items-center justify-center h-screen space-y-4">
    ...
    <div class="text-center">
      ...
      <p class="text-gray-600">Clique no canvas para desenhar um círculo. Clique em um círculo para selecioná-lo. Clique com o botão direito no canvas para ajustar o raio do círculo selecionado.</p>
    </div>

    <div class="canvas relative w-full h-full">
      <svg @click="onClick" class="w-full h-full">
        <circle
          v-for="circle in circles" :key="circle"
          :cx="circle.cx"
          :cy="circle.cy"
          :r="circle.r"
    ...
    <div class="controls text-center mt-4">
      ...
      <button @click="redo" :disabled="index >= history.length - 1" class="btn">Refazer</button>
    </div>

    <div v-if="adjusting" class="dialog fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white w-72 p-4 rounded-md shadow-md">
      <h3 class="text-lg font-semibold mb-2">Ajustar Raio</h3>
      <p class="mb-4">Ajuste o raio do círculo em ({{ selected.cx }}, {{ selected.cy }})</p>
      <input type="range" v-model="selected.r" min="1" max="300" class="w-full">
    </div>
  </div>
</template>
